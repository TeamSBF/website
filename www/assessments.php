...
$array = ["Chairstand" =>["30 Second Chair Stand","Lower body strength evaluation. Assess strength for climbing stairs, walking, and getting out of a chair, car, or tub. Number of full stands that can be completed in 30 seconds with arms folded across chest. If the arms are pulled away from the chest or you rock back and forward to help you stand that is unacceptable and the test will be stopped at that point. You may rest while siting on the chair and continue if you are still within the 30 seconds.","ChairStand","m0APvLqZr5E"],
             ...
             "leftunilateralbalancetest"=>["Unilateral Balance Test","Fall risk evaluation.  Balance test determined by how long you can stand on one foot without moving, or touching the lifted foot back to the ground.  The lifted leg may not be braced against the support leg, lift the lower leg up and to the rear till the knee is at 90 degrees. Not acceptable is excessive movement of arms or body to hold position","Unilateral","m0APvLqZr5E"],
...
$find->Select("Chairstand","ArmCurl","StepTest","FootUpAndGo","leftunilateralbalancetest","rightunilateralbalancetest","FunctionalReach")->From('assessments')->Where(['userID', '=', $user->id]);
$find->Limit();
    ...
    <form method="post">
        ...
        <?php 
            ...
            for($x=0;$x<count($keys);$x++){
                ...
                <h2><?php echo $array[$keys[$x]][0];?></h2>  
                <div>
                    <?php
                   if($rowCount<1)
                   {?>
                      <input type="checkbox" name="<?php echo $array[$keys[$x]][2];?>">Select if you would like to do 
                   <?php }
                    else
                    {?>
                        Your Results:<input type="text" name="<?php echo $keys[$x];?>" value="<?php if($result[$keys[$x]]>=0){echo $result[$keys[$x]];}?>">
                    <?php }?>
